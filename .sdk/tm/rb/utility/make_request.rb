...
module ForzamusicUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ForzamusicResponse.new({})
    ...
    if fetch_err
      ...
      response = ForzamusicResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ForzamusicResponse.new(fetched)
    else
