...
boru-driven command-line client **and** interactive REPL for the Forzamusic
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/forzamusic-cli)
make build
...
./forzamusic-cli load 1 album            # {id:1} shorthand
./forzamusic-cli load '{id:1}' album       # explicit match map

# 5. Override the API base URL for a single call
FORZAMUSIC_BASE=https://api.example.com ./forzamusic-cli load 1 album

# 6. No arguments -> interactive REPL
./forzamusic-cli
forzamusic> load 1 album
forzamusic> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/forzamusic-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/forzamusic-cli load 1 album
   ```
...
```sh
./forzamusic-cli load 1 album          # scalar shorthand for {id:1}
./forzamusic-cli load '{id:1}' album     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `forzamusic>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/forzamusic-cli`. |
...
(`./forzamusic-cli <expr>`) and interactively (the REPL), and expressions compose the
