...
[MCP](https://modelcontextprotocol.io) server exposing the Forzamusic SDK as
two agent tools — `forzamusic_list` and `forzamusic_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/forzamusic-mcp)
make build

# 2. Provide credentials via the environment
export FORZAMUSIC_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user forzamusic \
  -- /absolute/path/to/forzamusic-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./forzamusic-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// forzamusic_list: first page of records
{ "entity": "search" }
{ "entity": "search", "query": { } }

// forzamusic_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/forzamusic-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user forzamusic \
     -- "$PWD"/dist/*/forzamusic-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `forzamusic_list` and `forzamusic_load` tools now appear
   in new sessions. Ask the agent to *"list search using forzamusic"*
   and it calls `forzamusic_list` with `{"entity":"search"}`.
...
```sh
export FORZAMUSIC_APIKEY=sk_live_xxx            # API key
export FORZAMUSIC_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `forzamusic_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `forzamusic_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FORZAMUSIC_APIKEY` | API key sent with every request. |
| `FORZAMUSIC_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

album | lyric | search | song

### Smoke test via HTTP (raw JSON-RPC)

```sh
./forzamusic-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"forzamusic_load","arguments":{"entity":"album","query":{"id":1}}}}'
```
