...
func main() {
	...
	// Configure from the environment: FORZAMUSIC_APIKEY carries the API key and
	// FORZAMUSIC_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FORZAMUSIC_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FORZAMUSIC_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewForzamusicSDK(opts)
