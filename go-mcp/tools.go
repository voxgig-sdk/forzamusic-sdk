...
type Args struct {
	Entity string         `json:"entity" jsonschema:"album | lyric | search | song"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.ForzamusicSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "forzamusic_list",
		Description: "List records from Forzamusic. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "forzamusic_load",
		Description: "Load a single record from Forzamusic. " +
...
func runOp(client *sdk.ForzamusicSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.ForzamusicSDK, name string) (sdk.ForzamusicEntity, error) {
	switch strings.ToLower(name) {
	case "album":
		return client.Album(nil), nil
	case "lyric":
		return client.Lyric(nil), nil
	case "search":
