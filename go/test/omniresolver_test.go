...
import (
	...
	omni "github.com/voxgig-sdk/forzamusic-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ForzamusicError); is {
				out := map[string]any{
					"name":    "ForzamusicError",
...
func omniCtx(arg any, client *sdk.ForzamusicSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ForzamusicSDK); is {
			client = live
