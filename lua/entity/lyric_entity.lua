...
local LyricEntity = {}
LyricEntity.__index = LyricEntity


function LyricEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LyricEntity)
  self._name = "lyric"
  self._client = client
...
function LyricEntity:get_name()
  return self._name
end


function LyricEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LyricEntity.new(self._client, opts)
end
...
function LyricEntity:mark_deleted()
  self._deleted = true
end


function LyricEntity:deleted()
  return true == self._deleted
end


function LyricEntity:data_set(args)
  if args ~= nil then
...
function LyricEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LyricEntity:match_set(args)
  if args ~= nil then
...
function LyricEntity:match_get()
...
function LyricEntity:stream(action, args, callopts)
...
function LyricEntity:_run_op(ctx, post_done)
  local utility = self._utility
