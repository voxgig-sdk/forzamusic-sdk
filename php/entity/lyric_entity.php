...
class LyricEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new LyricEntity($this->_client, $opts);
    }

    /**
     * @param Lyric|array $args Lyric data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ForzamusicHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Lyric|array The current Lyric data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Lyric fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ForzamusicHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Lyric fields).
     */
     ...
     * Load a single Lyric.
     *
     * @param LyricLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed LyricLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Lyric|array The loaded Lyric as an assoc-array at the
     *   SDK boundary; throws ForzamusicError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ForzamusicHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
