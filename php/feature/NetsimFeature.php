...
class ForzamusicNetsimFeature extends ForzamusicBaseFeature
{
    ...
    private function _simulate(ForzamusicContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(ForzamusicContext $ctx, array $applied): void
    {
        $client = $this->client;
