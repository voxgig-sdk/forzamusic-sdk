...
class ForzamusicSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = ForzamusicHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = ForzamusicHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!ForzamusicFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, ForzamusicFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return ForzamusicUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = ForzamusicHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = ForzamusicHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = ForzamusicHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new ForzamusicError($op . "_allow",
                "ForzamusicSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = ForzamusicHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new ForzamusicError("graphql_error",
                "ForzamusicSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Lyric($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_lyric === null) {
                $this->_lyric = new LyricEntity($this, null);
            }
            return $this->_lyric;
        }
        return new LyricEntity($this, $data);
    }
