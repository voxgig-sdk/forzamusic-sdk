...
class AlbumEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FORZAMUSIC_TEST_ALBUM_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $album_ref01_data = null;
        if (count($album_ref01_data_raw) > 0) {
...
function album_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["album01", "album02", "album03"] as $k) {
    ...
    $entid_env_raw = getenv("FORZAMUSIC_TEST_ALBUM_ENTID");
