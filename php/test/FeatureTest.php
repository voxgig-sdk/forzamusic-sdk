...
class FtCtrl extends ForzamusicControl
{
...
class FtClient
{
    ...
    private ForzamusicUtility $utility;

    public function __construct(ForzamusicUtility $utility, array $options)
    {
    ...
    public function get_utility(): ForzamusicUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ForzamusicContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ForzamusicContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ForzamusicContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ForzamusicConfig::make_feature($name);
    ...
    private static function buildUrl(ForzamusicSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ForzamusicError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ForzamusicResponse($fetched);
        }
    ...
    private function fail(ForzamusicContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ForzamusicContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
