...
// Forzamusic SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = ForzamusicOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(ForzamusicOmni::structfix($now, Struct::undef()))
                        !== json_encode(ForzamusicOmni::structfix($val, Struct::undef()))) {
