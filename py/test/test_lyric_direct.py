...
class TestLyricDirect:

    def test_should_direct_load_lyric(self):
        setup = _lyric_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-lyric", "live" if setup["live"] else "unit")
        if _skip:
...
def _lyric_direct_setup(mockres):
