...
The Ruby SDK for the Forzamusic API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
client = ForzamusicSDK.test(testopts, sdkopts)
```
...
raise a `ForzamusicError` (a `StandardError` subclass), so wrap
...
| `artists` | `Array` | List of all artists involved |
...
```
rb/
├── Forzamusic_sdk.rb       -- Main SDK module
...
The main module (`Forzamusic_sdk`) exports the SDK class
