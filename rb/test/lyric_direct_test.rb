...
class LyricDirectTest < Minitest::Test
  def test_direct_load_lyric
    setup = lyric_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-lyric", setup[:live] ? "live" : "unit")
    if _should_skip
...
def lyric_direct_setup(mockres)
