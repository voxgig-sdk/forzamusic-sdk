...
//     (\`new ForzamusicSDK(...)\` / \`ForzamusicSDK.test(...)\` become
//     \`ForzamusicSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"album":{"test01":{"id":"test01"}},"lyric":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}},"song":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ForzamusicSDK(...)\` and any
// \`ForzamusicSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ForzamusicSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ForzamusicSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
